...
server.on('message', (msg, rinfo) => {
  ...
  MongoClient.connect(url, function(err, db) {
    ...
    if(arr.length >= 11){
      ...
      var jsonDat = {gid:arr[10].toString(),id:arr[0].toString(),airTemp:arr[4].toString(),airHumid:arr[5].toString(),pm1:arr[1].toString(),pm25:arr[2].toString(),pm10:arr[3].toString(),rain:arr[7].toString(),uv:arr[6].toString(),soilHumid:arr[8].toString(),wind:arr[9].toString(),dt:now.toLocaleString()};
      console.log(jsonDat);
      // try {
      //   JSON.parse(jsonDat);
      // } catch (e) {
